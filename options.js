...
// 从存储中加载设置
chrome.storage.sync.get(["selectedApi", "apiParams"], (data) => {
  apiSelection.value = data.selectedApi || "google";
...
function updateCustomParamsUI() {
  ...
  if (!apiParams.hasOwnProperty(apiSelection.value)) return;
...
saveButton.addEventListener("click", () => {
  ...
  chrome.storage.sync.set({ "selectedApi": apiSelection.value, "apiParams": apiParams }, () => {
    showMessage("设置已保存！");
  });
});
// 显示消息框并自动隐藏
